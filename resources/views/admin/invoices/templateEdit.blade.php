...
@section('menu_title')
    <li class="d-flex align-items-center">
        <div class="d-inline-block d-flex mr-auto align-items-center ml-1 lvl-1">
            <h4 class="my-0 me-2 menu-title text-white">{{ trans('messages.invoice_template') }}</h4>
...
@section('content')
    <form id="classic-builder-form" action="{{ action('Admin\InvoiceController@templateEdit') }}" method="POST" class="ajax_upload_form form-validate-jquery">
        {{ csrf_field() }}
        ...
        <div class="row mr-0 ml-0">
            <div class="col-md-9 pl-0 pb-0 pr-0 form-group-mb-0">
                ...
                @include('helpers.form_control', [
                    ...
                    'type' => 'textarea',
                    'name' => 'content',
                    'value' => Acelle\Model\Invoice::getTemplateContent(Auth()->user()->admin->getLanguageCode()),
                    ...
                    @foreach(Acelle\Model\Invoice::getTags() as $tag)
                        @if (!$tag["required"])
                            <a style="padding: 3px 7px !important;
                                font-weight: normal;" draggable="false" data-popup="tooltip" title='{{ trans('messages.click_to_insert_tag') }}' href="javascript:;" class="btn btn-primary mb-2 mr-1 text-semibold btn-xs insert_tag_button" data-tag-name="{{ $tag["name"] }}">
                                {{ $tag["name"] }}
                            </a>
                        @endif
                    @endforeach
